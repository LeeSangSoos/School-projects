...
from src.solver import UCS2
from src.solver import A_search
...
class MazeManager(object):
    ...
    def euclid(self, ax, ay, dx, dy):
        return ((ax-dx)**2+(ay-dy)**2)**(1/2)
    
    def solve_maze(self, maze_id, method, neighbor_method="fancy", episodes=100, a=0.1, r=0.1, e=0.5):
        ...
        if method == "DepthFirstBacktracker":
            ...
            solver = UCS2(maze, neighbor_method, self.quiet_mode)
            maze.solution_path, cost = solver.uniform_cost_search(maze)
        elif method == "A_search":
            solver = A_search(maze, neighbor_method, self.quiet_mode)
            maze.solution_path, cost = solver.a_star_search(maze, self.euclid)
        elif method == "Q_learning":
            solver = Q_learning(maze, neighbor_method, self.quiet_mode, episodes, a, r, e)
            maze.solution_path = solver.solve()
