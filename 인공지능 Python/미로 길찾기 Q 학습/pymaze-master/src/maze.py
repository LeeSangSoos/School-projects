...
class Maze(object):
    ...
    def find_neighbours3(self, cell_row, cell_col, cost):
        neighbours = list()

        def check_neighbour(row, col):
            # Check that a neighbour exists and that it's not visited before.
            if row >= 0 and row < self.num_rows and col >= 0 and col < self.num_cols:
                if(not self.grid[row][col].visited and not self.grid[row][col].is_walls_between(self.grid[cell_row][cell_col])):
                    neighbours.append((row, col, cost+1))
